...
use troves_clvaults::interfaces::IEkuboDistributor::Claim;
...
pub trait IClVault<TContractState> {
    // user functions (all pausable functions)
    fn deposit(
    ...
    // permissionless functions (pausable function)
    fn handle_fees(ref self: TContractState, pool_index: u64);

    // governor only (non-pausable functions)
    ...
    // relayer only (non-pausable functions)
    ...
    // liquidity and position functions
    ...
    fn get_max_unused_balances_on_rebalance(self: @TContractState) -> MaxUnusedBalances;
}
